...
10. Future development: Persistent memory in mex.
...
int open_port() {
    ...
    fd = open("/dev/cu.usbmodem12341", O_RDWR | O_NOCTTY | O_NDELAY);
    if (fd == -1) {
	    mexPrintf("Unable to open port. \n");
    }
    else {
...
unsigned long read_data(int fd) {
    memset(buf, 0, BUFLEN);
    read(fd, buf, BUFLEN);
    return strlen(buf);
}
